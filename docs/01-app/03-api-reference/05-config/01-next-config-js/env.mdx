...
For production deployments, Next.js recommends validating environment variables at build time using the built-in `env` validation schema. This prevents common deployment failures caused by missing or misconfigured variables.

> **Best practice**: Always validate that required environment variables are present at build time. This prevents common deployment failures caused by missing or misconfigured variables.
